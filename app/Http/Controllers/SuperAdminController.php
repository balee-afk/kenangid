...
class SuperAdminController extends Controller
{
    public function superadminDashboard()
    {
        ...
        return view('superadmin.adminhome', compact(
            'totalUsers',
            'totalAdmins',
            'totalSuperadmins',
            'totalTransactions',
    ...
    public function manageUsers()
    {
        ...
        return view('superadmin.manage-users', compact('users'));
    }
    ...
    public function storeUser(Request $request)
    {
        ...
        return redirect()->route('superadmin.manageUsers')->with('success', 'Pengguna berhasil ditambahkan.');
    }
    ...
    public function updateUser(Request $request, $id)
    {
        ...
        return redirect()->route('superadmin.manageUsers')->with('success', 'Pengguna berhasil diperbarui.');
    }
    ...
    public function deleteUser($id)
    {
        ...
        if ($user->usertype !== 'user') {
            return redirect()->route('superadmin.manageUsers')->with('error', 'Anda hanya dapat menghapus pengguna dengan tipe "user".');
        }
        ...
        $user->delete();
        return redirect()->route('superadmin.manageUsers')->with('success', 'Pengguna berhasil dihapus.');
    }
    ...
    public function transactions(Request $request)
    {
        ...
        if ($request->has('download_pdf')) {
            $pdf = PDF::loadView('superadmin.transactions_pdf', compact('transactions', 'reportType'));
            return $pdf->download('laporan_transaksi_' . $reportType . '.pdf');
        }

        return view('superadmin.transactions', compact('transactions', 'reportType'));
    }
    ...
    public function acceptTransaction($transactionId)
    {
        ...
        if ($transaction->status === 'accepted') {
            return redirect()->route('superadmin.transactions')->with('error', 'Transaksi ini sudah diterima.');
        }
        ...
        return redirect()->route('superadmin.transactions')->with('success', 'Transaksi diterima dan storage diperpanjang.');
    }
    ...
    public function notifications()
    {
        ...
        return view('superadmin.notifications', compact('notifications', 'users'));
    }
    ...
    public function storeNotification(Request $request)
    {
        ...
        return redirect()->route('superadmin.notifications')->with('success', 'Notifikasi berhasil dikirim.');
    }
    ...
    public function deleteNotification($id)
    {
        ...
        return redirect()->route('superadmin.notifications')->with('success', 'Notifikasi berhasil dihapus.');
    }
    ...
    public function manageAdmins()
{
    ...
    return view('superadmin.manage-admins', compact('admins'));
}
...
public function storeAdmin(Request $request)
{
    ...
    $admin = User::create([
        ...
        'email' => $request->email,
        'password' => Hash::make($request->password),
        'usertype' => 'admin', // Set tipe pengguna sebagai admin
    ]);

    return redirect()->route('superadmin.manageAdmins')->with('success', 'Admin berhasil ditambahkan.');
}
...
public function updateAdmin(Request $request, $id)
{
    ...
    return redirect()->route('superadmin.manageAdmins')->with('success', 'Admin berhasil diperbarui.');
}
...
public function deleteAdmin($id)
{
    ...
    // Cegah penghapusan admin jika ingin memastikan ada setidaknya satu admin
    $totalAdmins = User::where('usertype', 'admin')->count();
    if ($totalAdmins <= 1) {
        return redirect()->route('superadmin.manageAdmins')->with('error', 'Tidak dapat menghapus admin terakhir.');
    }

    $admin->delete();
    return redirect()->route('superadmin.manageAdmins')->with('success', 'Admin berhasil dihapus.');
}
